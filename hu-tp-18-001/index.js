...
// HISTORIA DE USUARIO: 18 - Comprar Artículos en Línea
// DESCRIPCION: Permite procesar la compra de artículos en línea utilizando Culqi, generar la boleta de venta con la SUNAT y enviarla al correo del cliente.
// PATH: /api/ventas/hu-tp-18
// METHODS: POST
...
export const handler = async (event) => {
    ...
    try {
        const { token, email, items, item_type } = JSON.parse(event.body);
        ...
        if (item_type === 'product') {
            ...
            const lastBoletaNumber = result.rows.length > 0 ? parseInt(result.rows[0].number_purchase) : 153;
            ...
            const reportName = `Reporte de Compra ${new Date().toISOString()}`;
            const reportResult = await pool.query(
                'INSERT INTO t_reports (name, expense_incurred) VALUES ($1, $2) RETURNING report_id',
                [reportName, totalAmount * 1.18] // Nombre del reporte único y monto total con IGV
            );
            ...
            if (items.length !== 1) {
                return {
                    ...
                    body: JSON.stringify({
                        success: false,
                        message: 'Solo se puede procesar una membresía a la vez.'
                    })
                };
            }

            const membershipId = items[0].membership_id; // Obtener solo el ID de la membresía

            const membershipResult = await pool.query(
                'SELECT membership_id, name, price FROM t_memberships WHERE membership_id = $1',
                [membershipId]
            );
            ...
            const totalAmount = membership.price; // Solo un precio, no es necesario reducir
            ...
            if (paymentResponse.statusCode !== 201) {
                return {
                    ...
                    body: JSON.stringify({
                        success: false,
                        message: parsedPaymentResponse.user_message || 'Error en la solicitud de pago'
                    }),
                };
            } else {
                return {
                    ...
                    body: JSON.stringify({
                        success: true,
                        message: parsedPaymentResponse.user_message || 'Su pago se ha completado correctamente'
                    }),
                };
            }
        }
